<?php
    ...
    $comments_query_result = $con->prepare("SELECT * FROM comments WHERE product_id=? ORDER BY created_at DESC");
    $comments_query_result->bind_param("i", $item_id);
    $comments_query_result->execute();
    $comments_query_result = $comments_query_result->get_result();
...
<head>
    ...
    <title>SecurePhotography - Product Detail</title>
</head>
...
<body>
    ...
    <div class="jumbotron jumbotron-fluid">
        <div class="container">
            <div class="row">
                <div class="col">
                    <?php echo "<img src='products/" . $row_product['image'] . "' class='img-fluid' >"; ?>
                </div>
                <div class="col">
                    <?php echo "<h1 class='display-4'>" . $row_product['brand'] . " " . $row_product['name'] . "</h1>"; ?>
                    <?php echo "<p class='lead'> " . $row_product['category'] . " </p>"; ?>
                    <?php echo "<p class='lead'> " . $row_product['price'] . "€" . " </p>"; ?>
                    ...
                    <?php if (!empty($comments)) : ?>
                        <?php foreach ($comments as $comment) : ?>
                            <li class="media" style="margin-bottom: 15px; word-break: break-all; word-wrap: break-word;">
                                ...
                                <?php
                                    $fname = '';
                                    foreach (glob("uploads/" . md5($comment['user_id'])  . '.*') as $filename) {
                                        $fname = $filename;
                                    }
                                    if ($fname == '') {
